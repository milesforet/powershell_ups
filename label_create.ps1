...
if($sp_list.length -eq 0){
    Write-Logs "No shipments to create"
    exit
}
...
$bearer_tok = Get-Content C:\Users\MForet\Documents\Automations\UPS_Powershell\auth\bearer.txt
...
:outer foreach($item in $sp_list){
    ...
    try {
        ...
        <# elseif ($ship_to_info.dept -lt 14) {
            Write-Logs "$($item["Title"]) - Too "
        } #>
        ...
        foreach($package in $shipping_result){
            ...
            [IO.File]::WriteAllBytes("C:\Users\MForet\Documents\Automations\UPS_Powershell\images\$count.jpg", [Convert]::FromBase64String($label))
            $count++
        }
        ...
        try{
            ...
            Remove-item "C:\Users\MForet\Documents\Automations\UPS_Powershell\images\*.jpg"

            $i = Add-PnPFile -Path "C:\Users\MForet\Documents\Automations\UPS_Powershell\images\$date-$name-$count.pdf" -Folder "Labels"
